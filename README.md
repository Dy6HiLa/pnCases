...
Проверка обновлений зашита в код и не требует настроек в `config.yml`.

Плагин проверяет этот GitHub-репозиторий:
- сначала последний GitHub Release;
- потом git tags;
- если там нет версии, `src/main/resources/plugin.yml` в ветке `master`.

Чтобы выпустить обновление, создай release/tag вида `v1.3.0` или подними `version:` в `plugin.yml`. Если версия выше установленной, сервер напишет в консоль и онлайн-админам с `pncases.admin`.
